<script>
  ...
  import { ultimaLectura } from '../lib/mockData.js'
  ...
  $: ultima = ultimaLectura(sensorVinculado)
  ...
  function handleVincular() {
    ...
    vincularSensor(siloId, sensorSeleccionado)
  ...
  function getAlertColor(tipo, val) {
    if (!val) return 'ok'
    if (tipo === 'temperature' && val > 30) return 'critical'
    if (tipo === 'temperature' && val > 26) return 'warning'
    if (tipo === 'humidity' && val > 18) return 'critical'
    if (tipo === 'humidity' && val > 15) return 'warning'
    if (tipo === 'co2' && val > 800) return 'critical'
    if (tipo === 'co2' && val > 600) return 'warning'
    return 'ok'
  }
</script>
...
<div class="page">
  ...
  {:else}
    ...
    <div class="breadcrumb">
      ...
      <span>/</span>
      <button on:click={() => push('/campo/' + silo.campoId)}>{campo?.nombre}</button>
      <span>/</span>
    ...
    <div class="sensor-section">
      ...
      {#if !sensorVinculado}
        <div class="no-sensor-card">
          ...
          {:else}
            <p class="hint">No hay sensores disponibles en este campo. Registrá uno primero desde la vista del campo.</p>
          {/if}
        </div>
      {:else}
        <div class="sensor-card">
          ...
          <button class="sensor-detail-link" on:click={() => push('/sensor/' + sensorVinculado.id)}>
            Ver detalle →
          </button>
        </div>
      {/if}
    ...
    {#if ultima}
      ...
      <div class="charts-section">
        ...
        <div class="charts-grid">
          <SensorChart lecturas={sensorVinculado.lecturas} tipo="temperature" />
          <SensorChart lecturas={sensorVinculado.lecturas} tipo="humidity" />
          <SensorChart lecturas={sensorVinculado.lecturas} tipo="co2" />
        </div>
      </div>
    {/if}
  {/if}
...
<style>
  ...
  .sensor-card {
    background: var(--white); border: 0.5px solid var(--green-300);
  ...
  .btn-outline {
    ...
    border: 1px solid var(--green-300); padding: 9px 18px;
    border-radius: var(--radius-md); font-size: 13px; font-weight: 600;
  }
